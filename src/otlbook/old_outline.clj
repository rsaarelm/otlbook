; TODO REMOVEME deprecated code
; Switch to new outline when it's done
(ns otlbook.old-outline
  (:require [clojure.core.match :refer [match]]
            [clojure.string :as str]
            [otlbook.util :as util])
  (:refer-clojure :exclude [print load]))

(defrecord DeprecatedOutline [head body])
  ...
  (let
   [edn  ; Normalize by adding implicit nil head if needed.
    (cond
      ...
      :else edn)]
    (if (string? edn)
      (DeprecatedOutline. edn [])
      (DeprecatedOutline. (first edn) (map edn->otl (rest edn))))))
  ...
  (let [[head rest :as parsed] (parse-head depth input)]
    (when parsed
      (let [[body rest] (parse-body depth [] rest)]
        [(DeprecatedOutline. head body) rest]))))
...
(defmethod print-method DeprecatedOutline [otl w]
  (let
...
(defn load
  ...
  (let
   [outline-paths
    ...
    (->> (outline-paths path)
         (map #(DeprecatedOutline. % (parse (slurp %))))
         (DeprecatedOutline. nil))))
